...
function OurServices() {
  ...
  return (
    <div className="w-[90%] mx-auto py-[6rem] max-w-[1280px]">
          ...
          <small className='text-center mb-[1rem] text-[.9rem] block'>our remarkable services</small>
          <div className='md:flex-row md:justify-center xl:w-[87%] xl:mx-auto flex-wrap flex gap-8 flex-col items-center '>

            {
              services.map((item, index)=>(
                <div key={index} className='transition ease-out duration-200 w-[19rem] lg:w-[17rem] xl:w-[19rem] max-w-[20rem] hover:bg-white hover:text-[#383f5a] hover:rounded hover:border-2 border-[#383f5a] bg-[#383F5A] text-white py-[1.4rem] px-[1rem] text-center'>
              <h5 className=' font-[mada] font-bold text-[1.3rem] lg:text-[1.1rem] xl:text-[1.3rem]'>
                {item.title}
              </h5>
              <p className='mt-[.5rem] font-[mada] font-light text-[.8rem]'>{item.para}</p>
              <div className='mt-4 flex justify-end'>
                <button className='font-bold'> {item.link} </button>
                </div>  
            </div>
              ))}
          </div>
