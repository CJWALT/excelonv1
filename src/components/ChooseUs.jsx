      ...
      <div className='z-10 relative w-[90%] max-w-[1280px] mx-auto py-[6rem]'>
        ...
        <div className='flex flex-col w-[100%] md:flex-row gap-[.8rem] '>
            <div className='flex flex-col md:justify-center lg:w-[70%] md:flex-row md:w-[62%] items-center md:flex-wrap gap-[1.6rem] md:order-1 order-2'>
               {data.map((item, index) =>(
                  <div key={index} className="border-2 border-[#383F5A] lg:w-[17rem] md:w-[12rem] w-[14rem] max-w-[19rem] p-4 flex flex-col transition-shadow duration-300 rounded  gap-[.4rem] chooseUs">
                  <span>
                  {item.icon} 
                  </span>
                  <h3 className='text-[1rem] md:text-[.9rem]  lg:text-[1.2rem] font-semibold font-[inter]'>{item.header} </h3>
                  <p className='text-justify lg:text-[1rem] text-[.9rem]'>{item.para}</p>
          </div> 
               ))} 
            ...
            <div className='lg:w-[40%] order-1 md:self-center flex flex-col md:w-[38%] gap-4'>
                ...
                <div >
                    <h2 className='text-right text-[1rem] md:text-[1.3rem] lg:text-[1.8rem] font-[limelight]'>Excelon Difference</h2>
